...
export class J832 {
  ...
  constructor(config: J832Config) {
    ...
    let signerOrProvider: ethers.JsonRpcProvider | ethers.Wallet = this.provider;
  ...
  private toBytes32(input: string): string {
    if (/^0x[0-9a-fA-F]{64}$/.test(input)) return input; // Already bytes32 hex
   ...
   */
  async createResource(params: { resourceId: string; enforceUniqueness?: boolean }) {
    if (!this.hasSigner()) throw new Error('Provide a privateKey for write actions.');
   ...
   */
  async registerChange(params: { resourceId: string; dataHash: string; changeType: ChangeType }) {
    if (!this.hasSigner()) throw new Error('Provide a privateKey for write actions.');
    const resourceId = this.toBytes32(params.resourceId);
    const dataHash = this.toBytes32(params.dataHash);
    if (params.changeType < 0 || params.changeType > 4) throw new Error('Invalid changeType');
   ...
   * @param resourceId Unique resource string (will be hashed)
   */
  async getLatestChange(resourceId: string): Promise<Change> {
   ...
   */
  async getHistoryRange(resourceId: string, start: number, count: number): Promise<Change[]> {
    const bytes32Id = this.toBytes32(resourceId);
    const results = await this.contract.getHistoryRange(bytes32Id, start, count);
    return results.map((r: any) => this.parseChange(r));
  }

  /**
   * Get the number of changes/versions for a resource.
   */
  async getVersionCount(resourceId: string): Promise<number> {
    const bytes32Id = this.toBytes32(resourceId);
  ...
  async getAdmins(resourceId: string): Promise<string[]> {
    const bytes32Id = this.toBytes32(resourceId);
    return await this.contract.getAdmins(bytes32Id);
  }

  /** Get number of admins for a resource. */
  async getAdminCount(resourceId: string): Promise<number> {
    const bytes32Id = this.toBytes32(resourceId);
  ...
  async proposeAddAdmin(resourceId: string, newAdmin: string) {
    if (!this.hasSigner()) throw new Error('Provide a privateKey for write actions.');
    const bytes32Id = this.toBytes32(resourceId);
  ...
  async approveAddAdmin(resourceId: string) {
    if (!this.hasSigner()) throw new Error('Provide a privateKey for write actions.');
    const bytes32Id = this.toBytes32(resourceId);
  ...
  async proposeRemoveAdmin(resourceId: string, adminToRemove: string) {
    if (!this.hasSigner()) throw new Error('Provide a privateKey for write actions.');
    const bytes32Id = this.toBytes32(resourceId);
  ...
  async approveRemoveAdmin(resourceId: string) {
    if (!this.hasSigner()) throw new Error('Provide a privateKey for write actions.');
    const bytes32Id = this.toBytes32(resourceId);
  ...
  async proposeTransferOwnership(resourceId: string, newOwner: string) {
    if (!this.hasSigner()) throw new Error('Provide a privateKey for write actions.');
    const bytes32Id = this.toBytes32(resourceId);
  ...
  async approveTransferOwnership(resourceId: string) {
    if (!this.hasSigner()) throw new Error('Provide a privateKey for write actions.');
    const bytes32Id = this.toBytes32(resourceId);
  ...
  async isAdmin(resourceId: string, address: string): Promise<boolean> {
    const bytes32Id = this.toBytes32(resourceId);
    return await this.contract.isAdmin(bytes32Id, address);
  }

  /** Get the owner of a resource. */
  async getResourceOwner(resourceId: string): Promise<string> {
    const bytes32Id = this.toBytes32(resourceId);
  ...
  async setResourceActiveStatus(resourceId: string, active: boolean) {
    if (!this.hasSigner()) throw new Error('Provide a privateKey for write actions.');
    const bytes32Id = this.toBytes32(resourceId);
  ...
  async isUniquenessEnforced(resourceId: string): Promise<boolean> {
    const bytes32Id = this.toBytes32(resourceId);
    return await this.contract.isUniquenessEnforced(bytes32Id);
  }

  /** Set (enforce or disable) uniqueness for a resource (admin only). */
  async setUniqueness(resourceId: string, enforceUniqueness: boolean) {
    if (!this.hasSigner()) throw new Error('Provide a privateKey for write actions.');
    const bytes32Id = this.toBytes32(resourceId);
  ...
  private parseChange(result: any): Change {
    return {
      version: Number(result.version),
      dataHash: result.dataHash,
      timestamp: Number(result.timestamp),
      actor: result.actor,
      changeType: Number(result.changeType) as ChangeType,
    };
  }
}
export { ChangeType };
